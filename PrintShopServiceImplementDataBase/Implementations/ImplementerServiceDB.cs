...
namespace PrintShopServiceImplementDataBase.Implementations
{
    public class ImplementerServiceDB : IImplementerService
    {
        ...
        public ImplementerServiceDB(PrintShopDbContext context)
        {
        ...
        {
            var indentsWorker = context.Implementers
            .Select(x => new
            {
                ImplId = x.Id,
                Count = context.Indents.Where(o => o.Status == IndentStatus.Выполняется && o.ImplementerId == x.Id).Count()
            })
            .OrderBy(x => x.Count)
            .FirstOrDefault();
