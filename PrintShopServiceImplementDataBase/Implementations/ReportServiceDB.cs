...
namespace PrintShopServiceImplementDataBase.Implementations
{
 ...
 var paragraphTable = document.Paragraphs.Add(Type.Missing);
               ...
               missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = Prints[i].PrintName;
                    table.Cell(i + 1, 2).Range.Text = Prints[i].Price.ToString();
                }
        ...
        public List<StocksLoadViewModel> GetStocksLoad()
        {
           ...
           (stock, stockCompList) =>
            new StocksLoadViewModel
            {
                ...
                Ingredients = stockCompList.Select(r => new Tuple<string,
        ...
        public void SaveCustomerIndents(ReportBindingModel model)
        {
           ...
           iTextSharp.text.Font.BOLD);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].PrintName, fontForCells));
                table.AddCell(cell);
